...
byte teda_flag = 0xB3;
byte radar_area_flag = 0xB9;
byte cloud_qtd_flag = 0xBA;
byte prediction_flag = 0xBE;
byte reset_flag = 0xBF;
byte prediction_prob_flag = 0xC0;
byte time_session_flag = 0xB8;
byte cloud_index_flag = 0xB6;

byte mmcloud_time_flag = 0xB5;
...
int count_points = 0;
int count_date = 0;
int count_radar_area = 0;
bool time_compared = false;

int dimension = 4;
int max_clusters = 3;

vector<double> data_mmcloud(2);
MMCloud mmcloud(2,3);
...
void processOBD(CBuffer* buffer)
{
  ...
  * 1. Read the time from the NODE-RED server
  ...
  */
  ...
  } else if (time_server_obtained && time_session == 0) {
  ...
  for (byte i = 0; i < sizeof(obdData) / sizeof(obdData[0]); i++) {
    ...
    if (obd.readPID(pid, value)) {
        ...
        if (count_radar_area == 4){
          float result = calculate_area(radar_data);
          // Serial.print("AREA ");
          // Serial.println(result);
          data_mmcloud[0] = result;
          data_mmcloud[1] = radar_data.engine;
          buffer->add(radar_area_flag, ELEMENT_FLOAT, &result, sizeof(result));
          count_radar_area = 0;

          gettimeofday(&mmcloud_start, NULL);
          string driver_classification = mmcloud.process_point(count_points, data_mmcloud);
          gettimeofday(&mmcloud_end, NULL);

          // calculate the time spent in the mmcloud process in microseconds
          float time_spent = (mmcloud_end.tv_sec - mmcloud_start.tv_sec) * 1000000 + mmcloud_end.tv_usec - mmcloud_start.tv_usec;

          buffer->add(mmcloud_time_flag, ELEMENT_FLOAT, &time_spent, sizeof(time_spent));

          int driver_classification_int = 0;
          if (driver_classification == "aggressive"){
            Serial.println("[DEBUG] AGGRESSIVE DRIVER");
            // beep(1000);
            driver_classification_int = 2;
          } else if (driver_classification == "normal"){
            Serial.println("[DEBUG] NORMAL DRIVER");
            driver_classification_int = 1;
          } else if (driver_classification == "cautious"){
            Serial.println("[DEBUG] DEFENSIVE DRIVER");
            driver_classification_int = 0;
          }

          buffer->add(cloud_index_flag, ELEMENT_INT32, &driver_classification_int, sizeof(driver_classification_int));
        }
